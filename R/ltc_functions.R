...
plot.ltc <- function(x, ...) {
  ...
  # Try to find info, but don't fail if not found
  info2 <- info %>%
    dplyr::filter(palette_name == attr(chromata, "name"))

  # Use palette name and color count if info not found
  if (nrow(info2) == 0) {
    title_text <- palette_name
    subtitle_text <- paste(length(chromata), "colors")
  } else {
    title_text <- info2$palette_name
    subtitle_text <- info2$bio
  }
...
bird <- function(chrom){
  ...
  if (nrow(info2) == 0) {
    title_text <- palette_name
    subtitle_text <- paste(length(chrom), "colors")
  } else {
    title_text <- info2$palette_name
    subtitle_text <- info2$bio
  }
