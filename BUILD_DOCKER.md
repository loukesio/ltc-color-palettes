...
This happens because Docker is using **cached layers from an old
Dockerfile**. The old Dockerfile didn’t install dependencies properly.
...
Docker caches each layer (step) in the Dockerfile. When you: 1. Build
with an old Dockerfile → Docker caches those steps 2. Update the
Dockerfile → Docker still uses old cached layers 3. Build again → Uses
old cache, doesn’t see new `install2.r` command

**Solution:** `--no-cache` forces Docker to rebuild everything from
scratch.
...
The GitHub Actions workflow automatically builds without cache issues
because it starts fresh every time. Once you push to GitHub, the
workflow will build correctly!
...
That’s totally fine! The GitHub Actions will: - ✅ Build the Docker
image automatically - ✅ Test all dependencies - ✅ Verify ltc works
correctly - ✅ Generate build reports
